...
## Report and Assistant endpoints

`POST /v1/reports` accepts `ReportRequest`, resolves the same completed or
partial Analysis modules by project, and deterministically generates an editable
DOCX without another model call. Its project-scoped download route is
`GET /v1/projects/{project_id}/reports/{report_id}/download`. Report metadata is
stored in SQLite and generated bytes use the same `YOUTHLM_ARTIFACT_DIR` root.
