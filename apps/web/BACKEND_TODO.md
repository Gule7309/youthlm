...
- [x] 新帳號不再 seed 缺乏資料支撐的教育／職訓數字、假檔名或 legacy transform/analysis 卡；舊草稿載入與保存時安全移除這兩種不可新增節點，保留使用者來源、成果與小幫手並重算卡片數。
...
- [x] 最終自動品質閘門通過：Python 193/193、前端 91/91、Ruff、typecheck、production build、資料 audit、diff whitespace 與 npm audit 0；同源 production smoke 的首頁、JS/CSS、health、ready 與資料目錄皆為 200。
...
- [x] 小幫手可保存需求，並為每個唯一來源各產生一項單一來源圖表草稿；不產生簡報草稿，也不執行 AI 分析或產生成果內容。
...
- [x] 本機瀏覽器可按預覽電子郵件與筆記本保存節點位置、來源／成果設定、小幫手已送出訊息、操作草稿、連線，以及政策雷達收合狀態與最近盤點紀錄；卡片大小目前固定。
