...
export function ReportResultPanel({ execution, onRetry }: ReportResultPanelProps) {
  ...
  const handleDownload = async () => {
    ...
    setDownloadError('');
    try {
      const blob = await downloadReport(execution.view, undefined, undefined, { signal: controller.signal });
  ...
  return (
    <section className="space-y-3 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3" aria-label="研析報告成果">
      ...
      >
        ...
        {downloadState === 'loading' ? '下載與驗證中…' : downloadError ? '重試下載 DOCX' : '下載可編輯 DOCX'}
      </button>
    </section>
  );
}
